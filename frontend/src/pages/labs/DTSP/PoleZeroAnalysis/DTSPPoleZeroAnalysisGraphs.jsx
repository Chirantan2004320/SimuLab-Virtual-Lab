...
import { LineChart as LineChartIcon, CircleDot, X, ShieldCheck } from "lucide-react";
  ...
  return (
    <section className="comparison-shell">
      <div className="sorting-sim-title-wrap" style={{ marginBottom: 18 }}>
        ...
        <div>
          <h2 className="sorting-sim-title">Z-Plane Visualization</h2>
          <p className="sorting-sim-subtitle">
            Visualize zeros, poles, axes, and the unit circle reference.
          </p>
        </div>
      </div>

      {zeros.length === 0 && poles.length === 0 ? (
        <div className="coding-empty-state">
          Run the simulation first to plot zeros and poles.
        </div>
      ) : (
        <>
          <div
                  ...
                  return (
                    <g key={`zero-${i}`}>
                      <circle
                        ...
                        strokeWidth="2.5"
                      />
                      <text x={x + 12} y={y - 10} fontSize="12" fill="#22c55e">
                        Z{i + 1}
                      </text>
                    </g>
                  );
                  ...
                  return (
                    <g key={`pole-${i}`}>
                      ...
                      <line
                        ...
                        strokeWidth="2.5"
                      />
                      <text x={x + 12} y={y - 10} fontSize="12" fill="#ef4444">
                        P{i + 1}
                      </text>
                    </g>
                  );
          ...
          <div className="sorting-stats-grid">
            <div className="sorting-stat-box">
              <span className="sorting-stat-label">Number of Zeros</span>
              <span className="sorting-stat-value">{zeros.length}</span>
            </div>

            <div className="sorting-stat-box">
              <span className="sorting-stat-label">Number of Poles</span>
              <span className="sorting-stat-value">{poles.length}</span>
            </div>

            <div className="sorting-stat-box">
              <span className="sorting-stat-label">Reference</span>
            ...
            <div className="sorting-stat-box">
              <span className="sorting-stat-label">Stability</span>
          ...
          {zeros.length > 0 && (
            <div className="overview-card" style={{ marginBottom: 18 }}>
              ...
              <div className="sorting-stats-grid" style={{ marginBottom: 0 }}>
                {zeros.map((z, i) => (
                  <div key={i} className="sorting-stat-box">
                    <span className="sorting-stat-label">Z{i + 1}</span>
                    <span className="sorting-stat-value" style={{ fontSize: "1rem" }}>
                      {formatComplex(z)}
                    </span>
          ...
          {poles.length > 0 && (
            <div className="overview-card" style={{ marginBottom: 18 }}>
              ...
              <div className="sorting-stats-grid" style={{ marginBottom: 0 }}>
                {poles.map((p, i) => (
                  <div key={i} className="sorting-stat-box">
                    <span className="sorting-stat-label">P{i + 1}</span>
                    <span className="sorting-stat-value" style={{ fontSize: "1rem" }}>
                      {formatComplex(p)}
                    </span>
                    <span className="sorting-stat-label">
          ...
          <div className="sorting-info-box">
            ...
            ○ = Zero | × = Pole | Circle = Unit Circle. Poles inside the unit circle indicate stability for causal discrete-time systems.
          </div>
        </>
      )}
    </section>
