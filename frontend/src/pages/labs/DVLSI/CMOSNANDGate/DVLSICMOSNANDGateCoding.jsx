...
const problemBank = [
  {
    id: 1,
    title: "NAND Logic Function",
    description:
      "Write a function NAND(A, B) that returns the logical NAND of two binary inputs."
  },
  {
    id: 2,
    title: "Truth Table Generator",
    description:
      "Write a function generateNANDTruthTable() that returns all input-output combinations for a 2-input NAND gate."
  },
  {
    id: 3,
    title: "Pull-Down Detection",
    description:
      "Write a function hasPullDownPath(A, B) that returns true only if both nMOS transistors conduct in series."
  }
];

export default function DVLSICMOSNANDCoding() {
  ...
  return !(A && B) ? 1 : 0;
    ...
    if (id === 2) {
      return `function generateNANDTruthTable() {
  return [
    { A: 0, B: 0, Y: 1 },
    { A: 0, B: 1, Y: 1 },
    { A: 1, B: 0, Y: 1 },
  ...
  const runCode = (problemId) => {
    ...
    try {
      ...
      if (problemId === 1) {
        ...
        const fn = new Function(`${code}; return NAND;`)();
        const output = fn(1, 1);
        resultText =
          typeof output === "number"
            ? `Function ran successfully. Example NAND(1,1) = ${output}`
            : "Return value should be 0 or 1.";
      } else if (problemId === 2) {
        // eslint-disable-next-line no-new-func
        const fn = new Function(`${code}; return generateNANDTruthTable;`)();
        const output = fn();
        ...
        resultText =
          typeof output === "boolean"
            ? `Function ran successfully. Example hasPullDownPath(1,0) = ${output}`
  ...
  return (
    <section className="card">
      ...
      <p>
        Practice implementing NAND logic and understanding pull-up and pull-down behavior
        in CMOS gate design.
      </p>
